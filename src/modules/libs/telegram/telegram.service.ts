...
export class TelegramService extends Telegraf {
	...
	public async onStart(@Ctx() ctx: any) {
		...
		if (token) {
			...
			if (hasExpired) {
				await ctx.reply(MESSAGES.tokenExpired)
			}
	...
	public async onFollows(@Ctx() ctx: Context) {
		...
		if (user && follows.length) {
			...
			const message = `<b>List of channels that you follow:</b>\n\n${followsList}`

			await ctx.replyWithHTML(message)
		} else {
			await ctx.replyWithHTML(
				'<b>You are not following any channels yet.</b>'
			)
		}
	}
	...
	public async streamStartMessage(chatId: string, channel: User) {
		await this.telegram.sendMessage(chatId, MESSAGES.streamStart(channel), {
			parse_mode: 'HTML'
		})
	}

	public async newFollowerMessage(chatId: string, follower: User) {
		const user = await this.findUserByChatId(chatId)

		await this.telegram.sendMessage(
			chatId,
			MESSAGES.newFollower(follower, user.follower.length),
			{
				parse_mode: 'HTML'
			}
		)
